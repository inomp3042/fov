...
createAncorPeerTx() {
	...
	set -x
	configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/${volmsp}anchors.tx -channelID $CHANNEL_NAME -asOrg ${volmsp}
	res=$?
	set +x
	if [ $res -ne 0 ]; then
		echo "Failed to generate anchor peer update for ${volmsp}..."
		exit 1
	fi
	echo
	done
