...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-vol1.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-vol2.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
...
## call the script to join create the channel and join the peers of vol1 and vol2
...
function networkDown() {
  # stop vol3 containers also in addition to vol1 and vol2, in case we were running sample to add vol3
...
#  docker-compose -f $COMPOSE_FILE_COUCH_ORG3 -f $COMPOSE_FILE_ORG3 down --volumes --remove-orphans
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf organizations/fabric-ca/vol1/msp organizations/fabric-ca/vol1/tls-cert.pem organizations/fabric-ca/vol1/ca-cert.pem organizations/fabric-ca/vol1/IssuerPublicKey organizations/fabric-ca/vol1/IssuerRevocationPublicKey organizations/fabric-ca/vol1/fabric-ca-server.db
    rm -rf organizations/fabric-ca/vol2/msp organizations/fabric-ca/vol2/tls-cert.pem organizations/fabric-ca/vol2/ca-cert.pem organizations/fabric-ca/vol2/IssuerPublicKey organizations/fabric-ca/vol2/IssuerRevocationPublicKey organizations/fabric-ca/vol2/fabric-ca-server.db
...
#    rm -rf addOrg3/fabric-ca/vol3/msp addOrg3/fabric-ca/vol3/tls-cert.pem addOrg3/fabric-ca/vol3/ca-cert.pem addOrg3/fabric-ca/vol3/IssuerPublicKey addOrg3/fabric-ca/vol3/IssuerRevocationPublicKey addOrg3/fabric-ca/vol3/fabric-ca-server.db


    # remove channel and script artifacts
    rm -rf channel-artifacts log.txt fov.tar.gz
...
# use this as the docker compose couch file for vol3
#COMPOSE_FILE_COUCH_ORG3=addOrg3/docker/docker-compose-couch-vol3.yaml
# use this as the default docker-compose yaml definition for vol3
#COMPOSE_FILE_ORG3=addOrg3/docker/docker-compose-vol3.yaml
#
